...
module.exports = async (req, res) => {
    try {
        ...
        // =========================
        // 1. PUT /api/complaints/:id/resolve
        // =========================
        if (rawUrl.includes("/resolve") || (req.method === "PUT" && userRole === "ADMIN")) {
            if (userRole !== "ADMIN") {
                return res.status(403).json({ error: "Admin authorization required" });
            }

            const matches = rawUrl.match(/\/complaints\/(\d+)\/resolve/);
            const complaintIdParam = matches ? matches[1] : (req.query.id || req.query[0]);
            ...
            await pool.query("UPDATE complaints SET status = 'RESOLVED' WHERE id = $1", [complaintId]);
        ...
        if (req.method === "GET") {
            let query = `
                SELECT
                    c.id, c.user_id AS "userId", c.booking_id AS "bookingId",
