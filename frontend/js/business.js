...
export const BusinessModule = {
  ...
  async loadMyCargo() {
    ...
    try {
      const cargoList = await API.get(`/cargo?businessId=${user.id}`);
      if (cargoList.length === 0) {
        container.innerHTML = `<tr><td colspan="6" style="text-align:center; color:#64748B;">No cargo posted yet. Click "Create Cargo" to post your first shipment.</td></tr>`;
        return;
      }
      ...
      container.innerHTML = cargoList.map(c => `
        <tr>
          ...
          <td><strong>${c.weight} Tons</strong></td>
          <td><span class="pill-badge" style="margin:0; font-size:0.7rem;">${c.status}</span></td>
          <td>
            <a href="matching.html?cargoId=${c.id}" class="btn btn-solid-dark" style="padding:4px 10px; font-size:0.75rem;">Find Trucks &rarr;</a>
          </td>
        </tr>
      `).join('');
    } catch (err) {
  ...
  async loadMyBookings() {
    ...
    try {
      ...
      if (bookings.length === 0) {
        container.innerHTML = `<tr><td colspan="6" style="text-align:center; color:#64748B;">No active or past bookings.</td></tr>`;
        return;
      }
      ...
      container.innerHTML = bookings.map(b => `
        <tr>
          ...
          <td>
            <a href="tracking.html?bookingId=${b.id}" class="btn btn-outline-dark" style="padding:4px 10px; font-size:0.75rem;">Track 📍</a>
          </td>
        </tr>
      `).join('');
    } catch (err) {
  ...
  initCreateForm() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      const cargoData = {
        ...
        requiredDeliveryDate: document.getElementById('requiredDeliveryDate').value,
        preferredVehicleType: document.getElementById('preferredVehicleType').value,
        specialHandling: document.getElementById('specialHandling').value
      };

      try {
        const saved = await API.post('/cargo', cargoData);
        alert(`Cargo created successfully! Search ID: C00${saved.id}`);
        window.location.href = `matching.html?cargoId=${saved.id}`;
      } catch (err) {
        alert(`Failed to create cargo: ${err.message}`);
      }
    });
  }
};
